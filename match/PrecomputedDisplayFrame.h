...
private:
    ...
    std::shared_ptr<ReachNetwork> m_originalMappedNetwork;
    std::shared_ptr<PointSearchTree> m_pointSearchTree;
    int m_highlightedReachId;
    int m_depth;

public:
    explicit PrecomputedDisplayFrame(const QString& name, const std::shared_ptr<PreComputedReachNetwork>& network, const std::shared_ptr<ReachNetwork>& originalMappedNetwork);
    ...
    [[nodiscard]] QGraphicsScene* getOriginalMappedScene() const;
